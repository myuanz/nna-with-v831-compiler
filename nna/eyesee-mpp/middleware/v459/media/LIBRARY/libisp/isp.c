...
int isp_reset(int dev_id, int ir_flag)
{
	int ret = 0;
    ...
    media_params.ir_flag = ir_flag;
	isp_params_parse(isp, &isp_ctx[dev_id].isp_ini_cfg, media_params.ir_flag, media_params.isp_sync_mode);
...
int isp_init(int dev_id)
{
	...
	ret = isp_params_parse(isp, &isp_ctx[dev_id].isp_ini_cfg, media_params.ir_flag, media_params.isp_sync_mode);
	if (ret < 0) {
		if (dev_id >= 1)
