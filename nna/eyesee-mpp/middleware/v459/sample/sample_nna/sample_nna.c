...
//人脸
   ...
   //alloc 6M continuous phy address for nna
    ...
    // int reg_base = (int)xreg_open(NNA_BASE_ADDRESS);
    nna_init(NNA_BASE_ADDRESS);
    printf(
        "nna_setup_buffer: nna_buf_phy: %x, test_info.nna_buf_vir: %x, nna_buf_size: %u \n", 
        test_info.nna_buf_phy,
        (unsigned int)test_info.nna_buf_vir,
        test_info.nna_buf_size
    );
...
// #endif
    ...
    // int *out = (int*)(test_info.nna_buf_vir);
    // int *input = out + 1;
    // printf("分配地址: input: %p, out: %p \n", input, out);

    // *out = 44;
    // nna_sdp_enable2(reg_base, 0, 1);
    // nna_conv_enable2(reg_base, 0, 1);

    // printf("nna_eltwise out: %d(%p) * %d(%p) = %d(%p) \n", input[0], input, input[1], input + 1, *out, out);
    // nna_eltwise((uint32_t)(out), (uint32_t)(input), (uint32_t)(input + 1), 0, 1, 1, 1, 0, 1, 1);
    // printf("nna_eltwise out: %d(%p) * %d(%p) = %d(%p) \n", input[0], input, input[1], input + 1, *out, out);
    ...
    int *out = (int*)(test_info.nna_buf_vir);
    int *input = out + len*len*3;
    int *weight = input + len*len;
    int *bias = weight + len*len;
    for (int i=0; i<len; i++){
        *(input + i) = i;
        *(weight + i) = i+1;
        *(bias + i) = i+2;
    ...
    printf("\n> nna_inner_product((uint32_t)out, (uint32_t)input, (uint32_t)weight, (uint32_t)bias, 0, 0, len, 1, 1, 1, 0);\n");
    nna_inner_product((uint32_t)out, (uint32_t)input, (uint32_t)weight, (uint32_t)bias, 0, 0, len, 1, 1, 1, 0);
    printf("\n> sleep(1)\n");
    sleep(1);
    for (int i=0; i<len; i++){
        printf("> %p input[%d] = %d\t", input + i, i, *(input + i));
        printf("%p weight[%d] = %d\t", weight + i, i, *(weight + i));
        printf("%p bias[%d] = %d\t", bias + i, i, *(bias + i));
        printf("%p output[%d] = %d\n", out + i, i, *(out + i));
    }

    for (int i=0; i<len; i++){
        for (int j=0; j<len; j++){
            *(input + i*len + j) = i*len + j;
            *(weight + i*len + j) = i*len + j + 1;
            *(bias + i*len + j) = i*len + j + 2;
            *(out + i*len + j) = INT32_MAX;
        }
    }

    printf("\n> nna_conv_act((uint32_t)out, (uint32_t)input, (uint32_t)weight, (uint32_t)bias, 0, 0, 0, 0x80808080, len, len, 1, 1, 2<<8+2, 2<<8+2, 1<<8+1, 0x01010101, 0, 0, 0);\n");
    nna_conv_act((uint32_t)out, (uint32_t)input, (uint32_t)weight, (uint32_t)bias, 0, 0, 0, 0x80808080, len, len, 1, 1, (2<<8)+2, (2<<8)+2, (1<<8)+1, 0x01010101, 0, 0, 0);
    printf("\n> sleep(1)\n");
    sleep(1);
    for (int i=0; i<len; i++){
        for (int j=0; j<len; j++){
            printf("> %p input[%d][%d] = %d\t", input + i, i, j ,*(input + i*len + j));
            printf("%p weight[%d][%d] = %d\t", weight + i, i, j ,*(weight + i*len + j));
            printf("%p bias[%d][%d] = %d\t", bias + i, i, j ,*(bias + i*len + j));
            printf("%p output[%d][%d] = %d\n", out + i, i, j, *(out + i*len + j));
        }
    }
    ...
    printf("\n> nna_conv_act_pool\n");
    nna_conv_act_pool(
        (uint32_t)out, (uint32_t)input, (uint32_t)weight, (uint32_t)bias, 0, 0, 0, 0x80808080, len, len, 1, 1, 
        (2<<8)+2, (2<<8)+2, (1<<8)+1, 0x01010101, 0, 0, 0, 0, (2<<8)+2, (1<<8)+1, (1<<8)+1
    );
    printf("\n> sleep(1)\n");
    sleep(1);
    for (int i=0; i<len; i++){
