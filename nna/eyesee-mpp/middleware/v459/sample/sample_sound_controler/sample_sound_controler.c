...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        fram_date_bak = nAFrame.mpAddr;//每次送到Sound CTR都是320Byte，所以备份下fram地址，在release时使用
        cmdcount = nAFrame.mLen/320;//得到fram长度不是取整数
        cmdcount_mod = nAFrame.mLen%320;//取余数，保证所有数据都会传入Sound CTR
        for(cmdcount;cmdcount >= 0;cmdcount--)
        {
            //alogd("cmdcount:%d,nAFrame.mLen:%d,nAFrame.mpAddr:%p",cmdcount,nAFrame.mLen,nAFrame.mpAddr);

            result = txzEngineProcess(cmdInst, nAFrame.mpAddr, 160, &cmdIndex,&cmd_confidence);
            if(cmdcount != 0)
            {
                nAFrame.mpAddr = nAFrame.mpAddr + 320;//地址后移320个字节
            }
            else
            {
                nAFrame.mpAddr = nAFrame.mpAddr + 128;
            }
