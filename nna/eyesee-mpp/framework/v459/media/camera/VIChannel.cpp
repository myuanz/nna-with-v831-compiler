...
void AdasInCallBack(ADASInData *ptrADASInData,void *dv)
{
    ...
    else
    {
        ...
        nADASInData.ptrFrameData=mVich->GetmpAdasThread()->ptrImgBuffer;//vvvvv:获取captureThread线程得到的图像数据
#if 0
...
void AdasInCallBack_v2(FRAMEIN &imgIn,void *dv)
{
    ...
    else
    {
        ...
        imgIn.imgSmall.ptr=mVich->GetmpAdasThread()->ptrImgBuffer;//vvvvv:获取captureThread线程得到的图像数据
        //printf("W:%d H:%d speed:%d\n",imgIn.imgSmall.imgSize.width,imgIn.imgSmall.imgSize.height,imgIn.gps.speed);
        #if 0
            printf("\n-capture picture data w:%d h:%d-\n",960,540);
            unsigned int data_size;
            FILE *fileNode = fopen("/mnt/extsd/adas.yuv", "wb+");
...
void AdasOutCallBack_v2(ADASOUT &adasOut,void *dv)
{
        ...
        else
        {
            if(NULL!=&adasOut)
            {
                ...
                //printf("\nw:%d,h:%d   22\n",mVich->GetmpAdasThread()->m_width,mVich->GetmpAdasThread()->m_height);
...
status_t VIChannel::DoAdasThread::startAdasDetect()
{
    ...
    ptrImgBuffer = (unsigned char *)malloc(m_width * m_height*3/2);
    if(ptrImgBuffer == NULL)
    {
        aloge("malloc ptrImgBuffer fail !!!");
...
bool VIChannel::DoAdasThread::AdasThread()
{
    ...
    PROCESS_MESSAGE:
        ...
        if(mbAdasDetectEnable)
        {
            ...
            {
                if(NULL != ptrImgBuffer)
                {
                    pthread_mutex_lock(&mDataTransferLock);
                    //copy data
                    //printf(",,,W:%d,H:%d\n\n",m_width,m_height);
                    memcpy(ptrImgBuffer, (unsigned char *)(pFrmbuf->mFrameBuf.VFrame.mpVirAddr[0]), m_width*m_height);
                    memcpy(ptrImgBuffer+m_width*m_height, (unsigned char *)(pFrmbuf->mFrameBuf.VFrame.mpVirAddr[1]), m_width*m_height/2);
                    //printf("\nmemcpy data to ptrImgBuffer  w:%d h:%d-\n",m_width,m_height);
